...
init_gpg()
{
    ...
    rm -fR "${RAMDISK}"
    mkdir "${RAMDISK}"
    _run 'ramdisk' mount -t tmpfs -o size=10m ramdisk "${RAMDISK}"
    ...
    ## Testing
    _message "ramdisk" "Testing ramdisk read/write"
...
cleanup_gpg_init()
{
    ...
    local GPG_TOMB_LABEL_ENC TOMB_SIZE KEYGRIP fingerprint

    GPG_TOMB_LABEL_ENC=$(_encrypt_filename "${IDENTITY}" "${GPG_TOMB_LABEL}" "$pass")
    TOMB_SIZE=15
    fingerprint=$(gpg -K "${email}" | grep fingerprint | head -n 1 | cut -d= -f2 | sed 's/ //g')
    ...
    open_tomb "${GPG_TOMB_LABEL}" "${IDENTITY}" "${pass}"
    ...
    close_tomb "${GPG_TOMB_LABEL}" "${IDENTITY}" "${pass}"
}
...
gpgpass ()
{
    local identity passphrase TIMEOUT

    identity="$(_identity_active_or_specified "${1}")"
